...
print("_____Statistiques Chèques v3_____")
...
# Niveau Avancé :
numero_montant_plus_faible, montant_plus_faible = 0, 0.0
numero_montant_plus_eleve, montant_plus_eleve = 0, 0.0
...
while numero_cheque > 0:
    ...
    if numero_cheque > 0:
        ...
        # Niveau Avancé : Comptabilisation du numéro et des montants le plus faible ou le plus élevé
        if nombre_cheques == 1 or montant_cheque <= montant_plus_faible:
            numero_montant_plus_faible = numero_cheque
            montant_plus_faible = montant_cheque
        if nombre_cheques == 1 or montant_cheque >= montant_plus_eleve:
            numero_montant_plus_eleve = numero_cheque
            montant_plus_eleve = montant_cheque
...
if nombre_cheques > 0:
    ...
    # Niveau Avancé :
    print(" - - - - - - - - - - - - - - - - - - - - - - - ")
    print(" • Montant du plus petit chèque :", montant_plus_faible, "€ ( n°", numero_montant_plus_faible, ")")
    print(" • Montant du plus grand chèque :", montant_plus_eleve, "€ ( n°", numero_montant_plus_eleve, ")")
else:
